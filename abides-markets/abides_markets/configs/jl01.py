...
def build_config(
    ...
    mm_min_order_size=1,
    mm_num_ticks=10,
    mm_wake_up_freq= 1000000,#str_to_ns("10S"),
    mm_skew_beta=0,
    mm_level_spacing=5,
    ...
    oracle = TradesOracle(mkt_open, mkt_close, symbols)
    ...
    noise_mkt_open = mkt_open - 1000000000
    noise_mkt_close = mkt_close + 1000000000
    agents.extend(
        [
            NoiseAgent(
                id=j,
            ...
            for j in range(agent_count, agent_count + num_noise) for symbol in symbols.keys()
        ]
    )
    agent_count += num_noise * len(symbols.keys())
    ...
    agents.extend(
        [
            ...
            for j in range(agent_count, agent_count + num_value) for symbol in symbols.keys()
        ]
    )
    agent_count += num_value * len(symbols.keys())
    ...
    agents.extend(
        [
            ...
            for idx, j in enumerate(range(agent_count, agent_count + num_mm_agents)) for symbol in symbols.keys()
        ]
    )
    agent_count += num_mm_agents * len(symbols.keys())
    ...
    agents.extend(
        [
            ...
            for j in range(agent_count, agent_count + num_momentum_agents) for symbol in symbols.keys()
        ]
    )
    ...
    # pov_agent_start_time = mkt_open
    # pov_agent_end_time = mkt_close
    ...
    kernelStopTime = mkt_close + 1000000
